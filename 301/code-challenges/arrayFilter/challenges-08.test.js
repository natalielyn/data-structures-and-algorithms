...
const oddValues = (arr) => {
  let oddNumArray = arr.filter( oddNum => {
    return oddNum % 2;
  });
  return oddNumArray;
};
...
const filterStringsWithVowels = (arr) => {
  let regex = /[aeiou]/
  let vowelsArray = arr.filter(vowels => {
    return regex.test(vowels);
  });
  return vowelsArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let forbiddenArray = arr.filter( values => {
   return !forbiddenValues.includes(values);
  });
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let snorlaxData = arr.filter( stats => {
...
const getStatName = (arr, minBaseStat) => {
  let objectArray = arr.filter( stats => {
    return stats.baseStat > minBaseStat;
  });
  objectArray = objectArray.map( stats => {
